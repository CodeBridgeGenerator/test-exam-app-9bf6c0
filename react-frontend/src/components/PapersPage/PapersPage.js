...
import PapersDatatable from "./PapersDataTable";
...
import PapersFakerDialogComponent from "./PapersFakerDialogComponent";
import PapersSeederDialogComponent from "./PapersSeederDialogComponent";
...
const PapersPage = (props) => {
    ...
    const filename = "papers.csv";

    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("papers");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                console.log({ error });
                setLoading(false);
                props.alert({ title: "Papers", type: "error", message: error.message || "Failed get Papers" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("papers").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Papers", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service("papers").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">Papers </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                    <Button label="add" loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="papers-add-button"/>
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="papers-datatable">
                    <PapersDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>
            ...
            <PapersEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <PapersCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <PapersFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <PapersSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(PapersPage);
