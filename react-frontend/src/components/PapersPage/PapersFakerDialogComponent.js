...
import papersFakerFactory from "./papersFakerFactory";
...
const PapersFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = papersFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("papers").create(elem));
...
export default connect(mapState, mapDispatch)(PapersFakerDialogComponent);
